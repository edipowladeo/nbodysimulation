SUBROUTINE CHAOS4(A, B, C, D, X1, Y1, X, Y)
!*******************************************************************************
!*																			   *
!*  DESCRIPTION FOR SUBROUTINE CHAOS4(A, B, C, D, X1, Y1, X, Y)                *
!*																			   *
!*  THIS SUBROUTINE CALCULATES POINTS FOR A CHAOTIC EQUATION WITH FOUR FOLD    *
...
!* CALCUALTE X
  XA = A*X1
  XB = B*X1*(X1**2+Y1**2)
  XC = C * (X1**4-(6.00E+00*X1**2*Y1**2)+Y1**4)
  XD = D*(X1**3 - 3.00E+00*X1*Y1**2)
  X = XA + XB + XC + XD

!* CALCULATE Y
  YA = A*Y1
  YB = B*Y1*(X1**2+Y1**2)
  YC = C * (4.00E+00*X1**3*Y1-4.00E+00*X1*Y1**3)
  YD = D*(-3.00E+00*X1**2*Y1+Y1**3)
  Y = YA + YB + YC + YD


  RETURN
END SUBROUTINE CHAOS4
