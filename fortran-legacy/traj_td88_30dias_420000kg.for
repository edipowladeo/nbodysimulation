...
!	Este programa calcula trajetórias nos problemas de 2, 3 e 4 corpos.  As
!     equações de movimento (12 ao todo) estão escritas no sistema de coorde-
...
!     Corpo 2 (Terra):
	...
	Vo2   =  Dsqrt(Mi1*( (2/Ro2) - (1/an2) ))
      ...
      X(4)  =  X(1) + Ro2*( Dcosd(OM2)*Dcosd(w2 + f2) 
	+              - Dsind(OM2)*Dsind(w2 + f2)*Dcosd(i2) )
	X(5)  =  X(2) + Ro2*( Dsind(OM2)*Dcosd(w2 + f2) 
	+              + Dcosd(OM2)*Dsind(w2 + f2)*Dcosd(i2) )
	X(6)  =  X(3) + Ro2*( Dsind(w2 + f2)*Dsind(i2) )
  
      V(4)  =  V(1) 
	+        - Dsqrt(Mi1/P2)*( Dcosd(OM2)*(Dsind(w2+f2) + e2*Dsind(w2))  
	+        + Dsind(OM2)*Dcosd(i2)*(Dcosd(w2+f2) + e2*Dcosd(w2)) )
	V(5)  =  V(2) 
	+        - Dsqrt(Mi1/P2)*( Dsind(OM2)*(Dsind(w2+f2) + e2*Dsind(w2))  
	+        - Dcosd(OM2)*Dcosd(i2)*(Dcosd(w2+f2) + e2*Dcosd(w2)) )
	V(6)  =  V(3) 
	+        + Dsqrt(Mi1/P2)*( Dsind(i2)*(Dcosd(w2+f2) + e2*Dcosd(w2))) 
!-----------------------------------------------------------------------------------
!     Corpo 3 (MUITO CUIDADO AQUI):
	...
	X3o   =  Ro3*(Dcosd(OM3)*Dcosd(w3 + f3)
  	+         - Dsind(OM3)*Dcosd(i3)*Dsind(w3 + f3))
	Y3o   =  Ro3*(Dsind(OM3)*Dcosd(w3 + f3) 
 	+         + Dcosd(OM3)*Dcosd(i3)*Dsind(w3 + f3))
	Z3o   =  Ro3*Dsind(i3)*Dsind(w3 + f3) 

	Vx3o  = - Dsqrt(Mi2/P3)*( Dcosd(OM3)*(Dsind(w3+f3) + e3*Dsind(w3))  
  	+        + Dsind(OM3)*Dcosd(i3)*(Dcosd(w3+f3) + e3*Dcosd(w3)) )
	Vy3o  = - Dsqrt(Mi2/P3)*( Dsind(OM3)*(Dsind(w3+f3) + e3*Dsind(w3))  
	+        - Dcosd(OM3)*Dcosd(i3)*(Dcosd(w3+f3) + e3*Dcosd(w3)) )
...
!     Corpo 4 (Sonda):
	...
	Vo4   =  Dsqrt(Mi2*( (2/Ro4) - (1/an4) ))  				   
      ...
      X4o   =  Ro4*(Dcosd(OM4)*Dcosd(w4 + f4)
  	+         - Dsind(OM4)*Dcosd(i4)*Dsind(w4 + f4))
	Y4o   =  Ro4*(Dsind(OM4)*Dcosd(w4 + f4) 
	+         + Dcosd(OM4)*Dcosd(i4)*Dsind(w4 + f4))
	Z4o   =  Ro4*Dsind(i4)*Dsind(w4 + f4)
	
      Vx4o  =	- Dsqrt(Mi2/P4)*( Dcosd(OM4)*(Dsind(w4+f4) + e4*Dsind(w4))  
 	+        + Dsind(OM4)*Dcosd(i4)*(Dcosd(w4+f4) + e4*Dcosd(w4)) )
	Vy4o  =	- Dsqrt(Mi2/P4)*( Dsind(OM4)*(Dsind(w4+f4) + e4*Dsind(w4))  
 	+        - Dcosd(OM4)*Dcosd(i4)*(Dcosd(w4+f4) + e4*Dcosd(w4)) )
...
!-----------------------------------------------------------------------------------
	...
	Pe4  = (2*PI*Dsqrt((an4**3)/Mi2))*(TN/(2*PI))
      ...
      Call RA15(X,V,TFINAL,PASSO,LL,NV,NCLASS,OUTPUT_STEP,FIXED,TINST,
	+          Cd,Ad,B,Phi,Lab,M1,M2,M3,M4,Elev,PERT)
...
!     ECI - equatorial para ECEF - equatorial
	...
	VxE   =   Vx24i*Dcos(OME) - X24i*WE*Dsin(OME)
     +   	    + Vy24i*Dsin(OME) + Y24i*WE*Dcos(OME)
      VyE   = - Vx24i*Dsin(OME) - X24i*WE*Dcos(OME)
     +        + Vy24i*Dcos(OME) - Y24i*WE*Dsin(OME)
      VzE   =   Vz24i
...
!     Da Lua e da sonda em relação à Terra:
	...
	Write(20,5) TA,R24i,V24i,En24,H24,a24,e24,i24,OM24,W24,f24,A24i,
	+            Rp24,Rap24,TB
...
!-----------------------------------------------------------------------------------
	...
	Else if(PERT == 2) then
	Call TD88(Xg,Yg,Zg,Vxg,Vyg,Vzg,TA,Cd,Ad,M4,Arx,Ary,Arz)
      Else if(PERT == 3) then
...
!     centro de massa dos corpos 1 e 2 (sistema Sol-Terra eem a Lua):
	...
	F(10)= (Mi1*(X1-X4)/R143)+(Mi2*(X2-X4)/R243)+(Mi3*(X3-X4)/R343)
    	+        + Aqx + Aprx + Apx  
	F(11)= (Mi1*(Y1-Y4)/R143)+(Mi2*(Y2-Y4)/R243)+(Mi3*(Y3-Y4)/R343)
	+        + Aqy + Apry + Apy 
	F(12)= (Mi1*(Z1-Z4)/R143)+(Mi2*(Z2-Z4)/R243)+(Mi3*(Z3-Z4)/R343)
	+        + Aqz + Aprz + Apz 
!-----------------------------------------------------------------------------------
...
!///////////////////////////////////////////////////////////////////////////////////
	...
	Double precision M2,Mip,Xg,Yg,Zg,Vxg,Vyg,Vzg,TA,  AGpx,AGpy,AGpz,
	+                 Acx,Acy,Acz,Ahx,Ahy,Ahz,Vx,Vy,Vz,ae,UC,FC,FV
	Double precision J2,J3,J4,J5,J6,J7,C22,S22,J22,L22, C31,S31, J31,
	+                 L31,C32,S32,J32,L32,C33,S33,J33,L33,C42,S42,J42,
     +                 L42,C44,S44,J44,L44,L
	Double precision X,Y,Z,R,R2,R3,R4,R5,R6,R7,R8,R9,R11,R13,R15,R17
	Double precision AJ2,AJ3,Aj4,Aj5,Aj6,Aj7,AJ22,AJ31,AJ32,AJ33,AJ42,
	+                 AJ44
	Double precision A2x,A2y,A2z,A3x,A3y,A3z,A4x,A4y,A4z,A5x,A5y,A5z,
	+                 A6x,A6y,A6z,A7x,A7y,A7z
	Double precision AJ22x,AJ22y,AJ22z,AJ31x,AJ31y,AJ31z,AJ32x,AJ32y,
	+                 AJ32z,AJ33x,AJ33y,AJ33z,AJ42x,AJ42y,AJ42z,AJ44x,
     +                 AJ44y,AJ44z,WE,OME,PI,V ,AC1,AC2
!-----------------------------------------------------------------------------------
...
!     para o sistema ECEF e, ainda, no sistmea SI.
	...
	WE   =   2*PI/86400                                !rad/s
      ...
      Vx   =	 (  Vxg*Dcos(OME) - Xg*WE*Dsin(OME) 
	+          + Vyg*Dsin(OME) + Yg*WE*Dcos(OME) )*1000/FV
      Vy   =   (- Vxg*Dsin(OME) - Xg*WE*Dcos(OME) 
	+          + Vyg*Dcos(OME) - Yg*WE*Dsin(OME) )*1000/FV
      Vz   =   (  Vzg )*1000/FV  
...
!     Aceleração para J5
	...
	A5z  = -AJ5*(( (315*(Z**3)/R11) - (231*(Z**5)/R13) 
	+       - (105*Z/R9) )*Z + 5/R7 )                     
...
!     Aceleração para J6
	...
	A6x  = -AJ6*(  (35/(3*R9)) - (315*(Z**2)/R11) + (1155*(Z**4)/R13) 
	+       - (1001*(Z**6)/R15) )*X
	A6y  = -AJ6*(  (35/(3*R9)) - (315*(Z**2)/R11) + (1155*(Z**4)/R13)
	+       - (1001*(Z**6)/R15) )*Y
	A6z  = -AJ6*( (245/(3*R9)) - (735*(Z**2)/R11) + (1617*(Z**4)/R13) 
	+       - (1001*(Z**6)/R15) )*Z
...
!     Aceleração para J7
	...
	A7x  = -AJ7*(  (105*Z/R11) - (1155*(Z**3)/R13) + (3003*(Z**5)/R15) 
	+       - (2145*(Z**7)/R17) )*X
	A7y  = -AJ7*(  (105*Z/R11) - (1155*(Z**3)/R13) + (3003*(Z**5)/R15) 
	+       - (2145*(Z**7)/R17) )*Y
      A7z  = -AJ7*( ((420*Z/R11) - (2310*(Z**3)/R13) + (4004*(Z**5)/R15) 
	+       - (2145*(Z**7)/R17) )*Z - (35/(3*R9)) )

!	Aceleração para J22

      AJ22  =  (3*Mip*J22*(ae**2))

	AJ22x =	-AJ22*( ( (3/R5) - (5*(Z**2)/R7) )*X*Dcosd(2*(L-L22)) 
	+      -2*((1/R3)-((Z**2)/R5))*(Y/(X**2 + Y**2))*Dsind(2*(L-L22)) )
     	AJ22y =	-AJ22*( ( (3/R5) - (5*(Z**2)/R7) )*Y*Dcosd(2*(L-L22)) 
	+      -2*((1/R3)-((Z**2)/R5))*(X/(X**2 + Y**2))*Dsind(2*(L-L22)) )
...
!     Aceleração para J31
	...
	AJ31x = -AJ31*( ( (30*(Z**2)/R8) - (4/R6) )*(Dsqrt(1 - (Z**2)/R2))
	+        -( (5*(Z**2)/R6) - (1/R4) )
     +        *( (Z**2)/(R4*Dsqrt(1 - (Z**2)/R2)) )*X*Dcosd(L-L31)
	+        -( (5*(Z**2)/R6) - (1/R4) )*(Dsqrt(1 - (Z**2)/R2))
     +        *( Y/(X**2 + Y**2) )*Dsind(L-L31) ) 
	AJ31y = -AJ31*( ( (30*(Z**2)/R8) - (4/R6) )*(Dsqrt(1 - (Z**2)/R2)) 
     +        -( (5*(Z**2)/R6) - (1/R4) )
	+        *( (Z**2)/(R4*Dsqrt(1 - (Z**2)/R2)) )*Y*Dcosd(L-L31)
	+        +( (5*(Z**2)/R6) - (1/R4) )*(Dsqrt(1 - (Z**2)/R2))
	+        *( X/(X**2 + Y**2) )*Dsind(L-L31) )
	AJ31z = -AJ31*( ( (30*(Z**2)/R8) - (14/R6))*(Dsqrt(1 - (Z**2)/R2))
	+        -( (5*(Z**2)/R6) - (1/R4) )
     +        *( ((Z**2)/R4) - (1/R2) )*( Z/(Dsqrt(1 - (Z**2)/R2)) )
     +        *Dcosd(L-L31) )
...
!     Aceleração para J32
	...
	AJ32x = -AJ32*( ( (5*Z/R7) - (7*(Z**3)/R9) )*X*Dcosd(2*(L-L32))
     +        -2*( (Z/R5) - ((Z**3)/R7) )*(Y/(X**2 + Y**2))
     +        *Dsind(2*(L-L32)) )
	AJ32y = -AJ32*( ( (5*Z/R7) - (7*(Z**3)/R9) )*Y*Dcosd(2*(L-L32))
     +        +2*( (Z/R5) - ((Z**3)/R7) )*(X/(X**2 + Y**2))
     +        *Dsind(2*(L-L32)) )      
	AJ32z = -AJ32*( ( (8*(Z**2)/R7) - (7*(Z**4)/R9) - (1/R5) )
     +        *Dcosd(2*(L-L32)) )
...
!     Aceleração para J33
	...
	AJ33x =  AJ33*( ( (4*X/R6)*( (1 - (Z**2)/R2)**(1.5) ) 
	+        -(3*X/R4)*( ((Z**2)/R4)*(Dsqrt(1 - (Z**2)/R2)) ) )
     +         *Dcosd(3*(L-L33))			
	+        -(3*Y/R4)*( (1 - (Z**2)/R2)**(1.5) )*(1/(X**2 + Y**2))
     +        *Dsind(3*(L-L33)) )
	AJ33y =  AJ33*( ( (4*Y/R6)*( (1 - (Z**2)/R2)**(1.5) )
	+		-(3*Y/R4)*( ((Z**2)/R4)*( Dsqrt(1 - (Z**2)/R2) ) ) )
     +        *Dcosd(3*(L-L33))
	+        -(3*X/R4)*( (1 - (Z**2)/R2)**(1.5) )*(1/(X**2 + Y**2))
     +        *Dsind(3*(L-L33)) )
 	AJ33z =  AJ33*( ( (4*Z/R6)*( (1 - (Z**2)/R2)**(1.5) ) 
	+        +(3*Z/R4)*( Dsqrt(1 - (Z**2)/R2) )*((Z**2)/R4 - 1/(R**2))
     +        *Dcosd(3*(L-L33)) ) )
...
!     Aceleração para J42
	...
	AJ42x = -AJ42*( ( 54*(Z**2)/R9 - 63*(Z**4)/R11 + 5/R7 )*X
	+        *Dcosd(2*(L-L42))
	+        +2*( 8*(Z**2)/R7 - 7*(Z**4)/R9 - 1/R5 )*(Y/(X**2 + Y**2))
	+        *Dsind(2*(L-L42)) )
	AJ42y = -AJ42*( ( 54*(Z**2)/R9 - 63*(Z**4)/R11 + 5/R7 )*Y
	+        *Dcosd(2*(L-L42))
	+        +2*( 8*(Z**2)/R7 - 7*(Z**4)/R11 - 1/R5 )*(X/(X**2 + Y**2))
	+        *Dsind(2*(L-L42)) )
	AJ42z = -AJ42*( ( 82*(Z**2)/R9 - 63*(Z**4)/R11 + 21/R7 )*Z
	+        *Dcosd(2*(L-L42)) )
...
!     Aceleração para J44
	...
	AJ44x = -AJ44*( ( (5*X/R7)*( (1 - (Z**2)/R2)**2 ) 
	+        -(4*X*(Z**2)/R9)*(1 - (Z**2)/R2) )*Dcosd(4*(L-L44))
	+        -(4/R5)*( (1 - (Z**2)/R2)**2 )*(Y/(X**2 + Y**2))
	+        *Dsind(4*(L-L44)) ) 
      
	AJ44y = -AJ44*( ( (5*Y/R7)*( (1 - (Z**2)/R2)**2 ) 
	+        -(4*Y*(Z**2)/R9)*(1 - (Z**2)/R2) )*Dcosd(4*(L-L44))
	+        +(4/R5)*( (1 - (Z**2)/R2)**2 )*(X/(X**2 + Y**2))
	+        *Dsind(4*(L-L44)) )  
 
	AJ44z = -AJ44*( ( (5*Z/R7)*( (1 - (Z**2)/R2)**2 )
	+        -(4*Z/R5)*(1 - (Z**2)/R2)*((Z**2)/R4 - 1/R2) ) 
     +        *Dcosd(4*(L-L44)) )
!-----------------------------------------------------------------------------------
!     Aceleração total

	Ahx   = 0 + A3x + A4x + A5x + A6x + A7x + AJ22x + AJ31x      
	+        + AJ32x + AJ33x + AJ42x + AJ44x
	Ahy   = 0 + A3y + A4y + A5y + A6y + A7y + AJ22y + AJ31y   
	+        + AJ32y + AJ33y + AJ42y + AJ44y
	Ahz   = 0 + A3z + A4z + A5z + A6z + A7z + AJ22z + AJ31z 
	+        + AJ32z + AJ33z + AJ42z + AJ44z
...
!-----------------------------------------------------------------------------------
	...
	End
!//////////////////////////////////////////////////////////////////////////////////////////
      Subroutine TD88(Xg,Yg,Zg,Vxg,Vyg,Vzg,Cd,Ad,M4,TM,Arx,Ary,Arz)

      Implicit None

	Double precision Xg,Yg,Zg,Vxg,Vyg,Vzg,Vg,Cd,Ad,M4, T,TM,TN, UC,FV,
	+                 FC,PI,C,Arx,Ary,Arz,Ax,Ay,Az,Vx,Vy,Vz,Deg
	Double precision Fex,Feb,Kp
	Double precision K10,K11,K12,K13,K20,K21,K22,K23, K30,K31,K32,K33,
	+                 K40,K41,K42,K43,K50,K51,K52,K53, K60,K61,K62,K63,
	+                 K70,K71,K72,K73
	Double precision p3,p4,p5,p6,p7,a1,a2,a3,a4,a5,a6,a7,a8, fx,fm,fo,
	+                 ko,h,DA
	Double precision h1,h2,h3,h4,h5,h6,h7,R,Phi,g1,g2,g3,g4,g5,g6,g7,d
!-----------------------------------------------------------------------------------

	DA  =      0.00000D+00                            !0 h de 01/01/ano 
	Fex	=    150.80000D+00                            !sfu (=10-22 W/m2Hz)
	Feb =     69.30000D+00 							  !sfu
      Kp  =	   4.33000D+00
!-----------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------
      ...
      R   = Dsqrt(Xg**2 + Yg**2 + Zg**2)	             !uc
	Phi = DAsind(Zg/R)								 !Aqui pode ter problema
	h   = ((R*UC) - 6371.2)                          !km	   CUIDADO
!-----------------------------------------------------------------------------------

	h1 = K10 + K11*Dexp((120 - h)/(29*1)) + K12*Dexp((120 - h)/(29*2))
	+         + K13*Dexp((120 - h)/(29*3)) 

	h2 = K20 + K21*Dexp((120 - h)/(29*1)) + K22*Dexp((120 - h)/(29*2))
	+         + K23*Dexp((120 - h)/(29*3)) 
  
  	h3 = K30 + K31*Dexp((120 - h)/(29*1)) + K32*Dexp((120 - h)/(29*2))
	+         + K33*Dexp((120 - h)/(29*3)) 

	h4 = K40 + K41*Dexp((120 - h)/(29*1)) + K42*Dexp((120 - h)/(29*2))
	+         + K43*Dexp((120 - h)/(29*3)) 

	h5 = K50 + K51*Dexp((120 - h)/(29*1)) + K52*Dexp((120 - h)/(29*2))
	+         + K53*Dexp((120 - h)/(29*3)) 
    
 	h6 = K60 + K61*Dexp((120 - h)/(29*1)) + K62*Dexp((120 - h)/(29*2))
 	+         + K63*Dexp((120 - h)/(29*3)) 

	h7 = K70 + K71*Dexp((120 - h)/(29*1)) + K72*Dexp((120 - h)/(29*2))
	+         + K73*Dexp((120 - h)/(29*3)) 
!-----------------------------------------------------------------------------------

	g1  = 1
	g2  = ( (fm/2) + a4 )
	g3  = Dsin(DA - p3)*Dsind(Phi)
	g4  = (a5*fm + 1)*Dsin(DA - p4)
	g5  = (a6*fm + 1)*Dsin(2*(DA - p5))
	g6  = (a7*fm + 1)*(Dsin(T - p6))*(Dcosd(Phi))
	g7  = (a8*fm + 1)*(Dsin(2*(T - p7)))*(Dcosd(Phi))**2
!-----------------------------------------------------------------------------------

      d   = fx*fo*ko*( h1*g1 + h2*g2 + h3*g3 + h4*g4 + h5*g5 + h6*g6 
	+               + h7*g7)
...
!-----------------------------------------------------------------------------------
	...
	Vz    =  (Vzg/FV)*1000							   !m/s
      ...
      Vg    =  Dsqrt(Vx**2 + Vy**2 + Vz**2)       	   !m/s

	C     =  (0.5)*Cd*d*Ad*(Vg**2)/(M4)	               !m/s^2
...
!///////////////////////////////////////////////////////////////////////////////////
	...
	Double precision M4,Ad,X12,Y12,Z12,X12i,Y12i,Z12i,X14,Y14,Z14,
	+                 X14i,Y14i,Z14i,R12i,R14i,Aprx,Apry,Aprz,Apx,
	+                 Apy,Apz,Ap
...
!     RA15T.FOR
!
      SUBROUTINE RA15(X,V,TF,XL,LL,NV,NCLASS,OS,FIXED,TINST,Cd,Ad,Be,
	+                Phi,Lab,M1,M2,M3,M4,Elev,PERT)
